...
const A4_PAGE_PX = 1122; // A4 à 96dpi : 297mm × (96/25.4) ≈ 1122px
...
export function HtmlCvPreview() {
  ...
  return (
    <div className="flex-1 overflow-auto bg-slate-200 p-4 sm:p-8 flex justify-center items-start custom-scrollbar">
      ...
      <div
        ref={paperRef}
        className={`relative w-full max-w-[850px] min-h-[1122px] bg-white rounded-xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.15)] overflow-hidden flex flex-col ${fontClass}`}
      >
        ...
        {/* Header */}
        <div
          className="text-white px-10 py-12 relative overflow-hidden"
        ...
        >
          ...
          <div className="relative z-10 flex items-center gap-8">
            ...
            <div className="flex flex-col gap-6 flex-1">
              ...
              {hasContactInfo && (
                <div className="flex flex-wrap gap-x-6 gap-y-3 mt-2 text-sm font-medium" style={{ color: "rgba(255,255,255,0.85)" }}>
                  {personalInfo.email && (
            ...
            {projects.length > 0 && (
              <section>
                ...
                <div className="grid grid-cols-1 gap-5">
                  {projects.map((proj) => (
                    <div key={proj.id} className="p-4 rounded-xl bg-slate-50 border border-slate-100 transition-colors hover:border-slate-200">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="text-base font-bold text-slate-900">{proj.name}</h4>
                        {proj.link && (
                          <a href={proj.link} target="_blank" rel="noopener noreferrer" className="text-xs font-medium transition-colors" style={{ color: theme.accentColor }}>
                            Voir ↗
                          </a>
                        )}
                      </div>
